...
def play_one_game(rl_agent, opponent, board_size, rl_stone):
    # play one episode, collect RL transitions
...
def assign_rewards(transitions, winner, rl_stone):
    # assign shaped + terminal rewards to transitions
    rewarded = []
...
def train(
    # train RL agent with epsilon decay and self/other play
    # loop: play game -> assign rewards -> update weights
...
def evaluate_vs(weights_path, opponent_type="random", board_size=9, games=100, seed=42):
    # evaluate trained agent vs opponent
